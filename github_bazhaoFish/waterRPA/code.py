...
def mouseClick(clickTimes,lOrR,img,reTry):
    # ! | 其他层级代码调用执行cmd.xls以及pic路径报错，运行路径改变
    img = dir_path+img

    #特殊处理 三击 confidence = 0.6
    if clickTimes == 3:
        while True:
            print("Specila point !")
...
def mainWork(sheet1):
    ...
    while i < sheet1.nrows:
        ...
        #7代表三击        
            ...
            if sheet1.row(i)[2].ctype == 2 and sheet1.row(i)[2].value != 0:
                reTry = sheet1.row(i)[2].value
            mouseClick(3,"left",img,reTry)
            print("三击左键",img)             
        i += 1

def bzy_main()->int:
    file = dir_path+'cmd.xls'
    try:
        wb = xlrd.open_workbook(filename=file)
        sheet1 = wb.sheet_by_index(0)
    except Exception as e:
        print("[ERR] Failed to open cmd.xls")
        print("[ERR]",e)
        return -1
